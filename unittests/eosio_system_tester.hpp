...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      create_accounts({ N(gocio.token), N(gocio.ram), N(gocio.ramfee), N(gocio.stake),
               N(gocio.bpay), N(gocio.vpay), N(gocio.saving), N(gocio.names), N(gocio.gns), N(gocio.gstake), N(gocio.vs) });


      produce_blocks( 100 );

      set_code( N(gocio.token), eosio_token_wast );
      set_abi( N(gocio.token), eosio_token_abi );

      {
         const auto& accnt = control->db().get<account_object,by_name>( N(gocio.token) );
         abi_def abi;
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(gocio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(gocio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(gocio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(gocio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(gocio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "gocio", "gocio.msig", core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(gocio.msig), eosio_msig_wast );
         set_abi( N(gocio.msig), eosio_msig_abi );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(gocio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "gocio", "alice1111111", core_from_string("650000000.0000"), "gocio" );
