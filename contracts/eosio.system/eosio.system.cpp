...
namespace eosiosystem {
    ...
    _rammarket(_self,_self)
   {
      ...
      if( itr == _rammarket.end() ) {
         auto system_token_supply   = eosio::token(N(gocio.token)).get_supply(eosio::symbol_type(system_token_symbol).name()).amount;
   ...
   void system_contract::bidname( account_name bidder, account_name newname, asset bid ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(gocio.token), {bidder,N(active)},
                                                    { bidder, N(gocio.names), bid, std::string("bid name ")+(name{newname}).to_string()  } );
      ...
      if( current == bids.end() ) {
         ...
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(gocio.token), {N(gocio.names),N(active)},
                                                       { N(gocio.names), current->high_bidder, asset(current->high_bid),
...
EOSIO_ABI( eosiosystem::system_contract,
     ...
     (buyrambytes)(buyram)(sellram)(delegatebw)(undelegatebw)(calcvrewards)(lockbw)(unlockbw)(refund)(gocreward)
     // voting.cpp
     (regproducer)(unregprod)(voteproducer)(regproxy)
     // producer_pay.cpp
     (onblock)(claimrewards)(gnrewards)
     // governance.cpp
