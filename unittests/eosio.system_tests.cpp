...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   transfer( "gocio", "alice1111111", core_from_string("1000.0000"), "gocio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "gocio", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(gocio.ram));
   const asset initial_ramfee_balance = get_balance(N(gocio.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_from_string("199.0000"), get_balance(N(gocio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_from_string("1.0000"), get_balance(N(gocio.ramfee)) );
   ...
   transfer( "gocio", "alice1111111", core_from_string("100000000.0000"), "gocio" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   const auto init_eosio_stake_balance = get_balance( N(gocio.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_from_string("300.0000"), get_balance( N(gocio.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance, get_balance( N(gocio.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   issue( "gocio", core_from_string("1000.0000"), config::system_account_name );
   issue( "gocio.stake", core_from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   //eosio stakes for alice with transfer flag

   transfer( "gocio", "bob111111111", core_from_string("1000.0000"), "gocio" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "gocio", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, eosio_system_tester ) try {
   ...
   auto key =  fc::crypto::public_key( std::string("GOC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") );
   ...
   auto key2 =  fc::crypto::public_key( std::string("GOC5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(gocio.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(gocio.saving)).get_amount();
      ...
      const int64_t expected_new_token = int64_t( ( initial_supply.get_amount() * double(secs_between_fills) * continuous_rate ) / secs_per_year );
      const int64_t expected_to_producers = int64_t( ( initial_supply.get_amount() * double(secs_between_fills) * continuous_rate / 5. ) / secs_per_year );
      const int64_t expected_to_voters = int64_t( ( initial_supply.get_amount() * double(secs_between_fills) * continuous_rate / 10. ) / secs_per_year );
      const int64_t expected_to_gns = int64_t( ( initial_supply.get_amount() * double(secs_between_fills) * 3 * continuous_rate / 20. ) / secs_per_year );
      const int64_t expected_saving = expected_new_token - expected_to_producers - expected_to_voters - expected_to_gns;

      BOOST_REQUIRE_EQUAL(expected_new_token,
                          supply.get_amount() - initial_supply.get_amount());

      //GOC changed from save 80% to 55%, 4/5 to 11/20
      BOOST_REQUIRE_EQUAL( expected_saving,
                          savings - initial_savings);
      BOOST_REQUIRE_EQUAL(int64_t( expected_to_producers / 4.) ,
   ...
   // wait 5 more minutes, defproducera can now claim rewards again
   {
      ...
      const int64_t expected_new_token = int64_t( ( initial_supply.get_amount() * double(usecs_between_fills) * continuous_rate ) / usecs_per_year );
      const int64_t expected_to_producers = int64_t( ( initial_supply.get_amount() * double(usecs_between_fills) * continuous_rate / 5. ) / usecs_per_year );
      const int64_t expected_to_voters = int64_t( ( initial_supply.get_amount() * double(usecs_between_fills) * continuous_rate / 10. ) / usecs_per_year );
      const int64_t expected_to_gns = int64_t( ( initial_supply.get_amount() * double(usecs_between_fills) * 3 * continuous_rate / 20. ) / usecs_per_year );
      const int64_t expected_saving = expected_new_token - expected_to_producers - expected_to_voters - expected_to_gns;
      ...
      int64_t to_producer        = expected_to_producers;
   ...
   // test stability over a year
   {
      ...
      //GOC save 55%, means 2.75% of total
      BOOST_REQUIRE(500 * 10000 > int64_t(double(initial_supply.get_amount()) * double(0.05)) - (supply.get_amount() - initial_supply.get_amount()));
      BOOST_REQUIRE(500 * 10000 > int64_t(double(initial_supply.get_amount()) * double(0.0275)) - (savings - initial_savings));
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(gocio.bpay));
      const asset    initial_vpay_balance      = get_balance(N(gocio.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(gocio.bpay));
      const asset    vpay_balance      = get_balance(N(gocio.vpay));
      ...
      //GOC changed 
      //0.5%
      const int64_t expected_goc_gn_saving = int64_t( double(initial_supply.get_amount()) * double(usecs_between_fills) * (cont_rate/ 10.) / usecs_per_year );
      //0.75%
      const int64_t expected_goc_voter_saving = int64_t( double(initial_supply.get_amount()) * double(usecs_between_fills) * (3 * cont_rate/ 20.) / usecs_per_year );

      BOOST_REQUIRE_EQUAL( int64_t(expected_supply_growth), supply.get_amount() - initial_supply.get_amount() );

      //GOC changed to new_tokens - to_producers(20%) - to_voters(10%) - to_gns(15%)
      BOOST_REQUIRE_EQUAL( int64_t(expected_supply_growth) - int64_t(expected_supply_growth)/5 - expected_goc_gn_saving - expected_goc_voter_saving , savings - initial_savings );
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of gocio"),
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "gocio", "alice1111111", core_from_string("600000000.0000"), "gocio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "gocio", "nofail", core_from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   {
      const asset initial_names_balance = get_balance(N(gocio.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_from_string("0.1001"), get_balance(N(gocio.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE(goc_reward_test, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto rewards = get_rewards_info( N(bob111111111), (uint64_t)0 );
   BOOST_REQUIRE_EQUAL(core_from_string("0.0000"), rewards["rewards"].as<asset>());
   ...
   {
      uint64_t             goc_governance_vote_period = 24 * 3600 * 7;  // 7 days
      uint64_t             goc_bp_vote_period = 24 * 3600 * 7;  // 7 days
      uint64_t             goc_vote_start_time = 24 * 3600 * 3;  // vote start after 3 Days
      produce_block( fc::seconds(goc_governance_vote_period + goc_bp_vote_period + goc_vote_start_time - 5*60 ) );
      ...
      auto rewards = get_rewards_info( N(bob111111111), (uint64_t)0 );
      BOOST_REQUIRE_EQUAL(0, rewards["rewards"].as<asset>().get_amount());
      ...
      auto usecs_between_fills = claim_time - initial_claim_time;
      int64_t expected_to_gns = int64_t( ( initial_supply.get_amount() * double(usecs_between_fills) * 3 * continuous_rate / 20. ) / usecs_per_year );

      rewards = get_rewards_info( N(bob111111111), (uint64_t)0 );
      // every single proposal have max reward limit
      const int64_t goc_max_proposal_reward = 500'000'0000;
      const int64_t goc_max_prop_reward_per_voter = 50'000'0000;
      ...
      const asset initial_balance = get_balance(N(bob111111111));
      BOOST_REQUIRE_EQUAL(initial_balance.get_amount(), 0);

      produce_blocks(50);
      BOOST_REQUIRE_EQUAL(success(), push_action(N(bob111111111), N(gocreward), mvo()("owner", "bob111111111")));
      const asset balance = get_balance(N(bob111111111));
      BOOST_REQUIRE_EQUAL(balance.get_amount(), expected_to_gns);

      // ask for gn reward again
      produce_blocks(5000);  //avoid  billed CPU time (2000 us) is greater than the maximum billable CPU time for the transaction (1868 us)
      BOOST_REQUIRE_EQUAL(success(), push_action(N(bob111111111), N(gocreward), mvo()("owner", "bob111111111")));
      const asset balance2 = get_balance(N(bob111111111));
      BOOST_REQUIRE_EQUAL(balance2.get_amount(), expected_to_gns);
      ...
      rewards = get_rewards_info( N(bob111111111), (uint64_t)0 );
      BOOST_REQUIRE_EQUAL(expected_to_gns, rewards["rewards"].as<asset>().get_amount());
      const asset balance3 = get_balance(N(bob111111111));
      BOOST_REQUIRE_EQUAL(balance3.get_amount(), expected_to_gns);
   }
} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE(goc_multiple_proposal_reward_test, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), goc_stake("carol1111111") );
   BOOST_REQUIRE_EQUAL( success(), goc_vote("carol1111111", 0, true) );

   auto rewards_b_0 = get_rewards_info( N(bob111111111), (uint64_t)0 );
   BOOST_REQUIRE_EQUAL(core_from_string("0.0000"), rewards_b_0["rewards"].as<asset>());
   auto rewards_b_1 = get_rewards_info( N(bob111111111), (uint64_t)1 );
   BOOST_REQUIRE_EQUAL(core_from_string("0.0000"), rewards_b_1["rewards"].as<asset>());
...

   {
      ...
      uint64_t             goc_bp_vote_period = 24 * 3600 * 7;  // 7 days
      uint64_t             goc_vote_start_time = 24 * 3600 * 3;  // vote start after 3 Days
      produce_block( fc::seconds(goc_governance_vote_period + goc_bp_vote_period + goc_vote_start_time) );
      ...
      const asset initial_balance_c = get_balance(N(carol1111111));
      BOOST_REQUIRE_EQUAL(initial_balance_c.get_amount(), 0);
